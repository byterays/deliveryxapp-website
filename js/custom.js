...
jQuery(document).ready(function () {
	...
	// 	jQuery('html,body').scrollTo("#section-tracking"); 
    // });
...
$(document).ready(function () {
    ...
    updateMxUnit();
    function updateMxUnit() {
        $("#dimension_unit").change(function () {
            //mxUnit=$(this).val();
            var displayUnit = $(this).val() == "Inch" ? "In" : "Ft";
            $(".mx-unit").text(displayUnit);
        });
    };

    onRequestEstimateCost();
    function onRequestEstimateCost() {
        $("#estimate-shipping-cost").on("submit", function (event) {
            event.preventDefault();
            $("#opt-callback").modal('show');
        });   
        
        $("#query-contact-detail").on("shown.bs.modal", function(){
            $("#estimate-result").modal('hide');
        });
    }

    updateQueryContactInfo();
    function updateQueryContactInfo() {

        $("#query-contact-detail").on("submit", function (event) {
            event.preventDefault();
            ...
            $.ajax({
                ...
                if(data.success){
                    ...
                    var form = $("#estimate-shipping-cost");
                    ...
                    $.ajax({
                        ...
                        complete: function (response) {
                            ...
                            //console.log(result.location[0].facility_city);
                            $("#pickup-location").text(result.location[0].facility_city + " - "+result.location[0].pincode + ", " + result.location[0].facility_state);
                            $("#delivery-location").text(result.location[1].facility_city + " - "+result.location[1].pincode + ", " + result.location[1].facility_state);
                            $("#estimated-cost").text(result.data.selected_data.cl_grand_total);  
                            $("#num-box").text(result.data.selected_data.ttl_box);
                            $("#total-weight").text(result.data.selected_data.actual_weight);
                                          
                            $("#estimate-result").modal('show')
                            .on('shown.bs.modal', function(){
                                $("#opt-callback").modal("hide");
                            });
                        }    
                    });
    ...
    ctaRequestContact();
    function ctaRequestContact(){
        $(".cta-contact").click(function(){
            $("#cta-request-confirm").modal('show');
            $("#estimate-result").modal('hide');            
        });
    }
